...
\begin{to_lem} 
    \label{lem_6.44}
    Всякую гладкую функцию, определенную в некоторой окрестности $x_0 \in \mathbb{R}^n$, в возможно меньшей окрестности $x_0$, можно представить с гладкими $g_k$ в виде
    \begin{equation*}
         f(x) = f(x_0) + \sum_{k=1}^n g_k(x)
         (x^k  - x_0^k) , 
         \hspace{1cm} 
         g_k \to \partial_k f|_{x}, \text{ при } x \to x_0.
     \end{equation*} 
\end{to_lem}
